...
/**
 * OpenAI-style tool definitions for vault search, read, and apply-gated writes.
 */
export function getToolDefinitions(includeSearch: boolean): ChatTool[] {
...
function proposeNote(
	...
	const path = asString(field(rawArgs, 'path'));
	const content = asString(field(rawArgs, 'content'));
	if (!path || content === undefined) {
		return { type: 'text', text: `${action} requires path and content.` };
	}
	const sanitized = sanitizeVaultPath(path);
	if (!sanitized) {
		return { type: 'text', text: 'Invalid path. Use a relative vault path to a markdown note.' };
	}
	if (action === 'update' && !resolveMarkdownFile(plugin.app, sanitized)) {
		return { type: 'text', text: 'Note not found. Use propose_create_note for a new file.' };
	}
	const proposal: NoteProposal = { action, path: sanitized, content };
	return {
		type: 'proposal',
		proposal,
		text: `Proposal recorded for ${action} at ${sanitized}. The user will review it in chat and must click Apply before the file is written. Do not claim the note was saved.`,
	};
}
